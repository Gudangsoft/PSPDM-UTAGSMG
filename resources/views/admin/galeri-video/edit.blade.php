    ...
    <div class="col-lg-7">
        <div class="admin-card card">
            ...
            <div class="card-body p-4">
                ...
                <form action="{{ route('admin.galeri-video.update', $video) }}" method="POST">
                    @csrf @method('PUT')
                    <div class="mb-3">
                        ...
                        <input type="url" name="url" id="url" class="form-control @error('url') is-invalid @enderror"
                            value="{{ old('url', $video->url) }}" placeholder="https://www.youtube.com/watch?v=..." oninput="detectPlatform(this.value)" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $video->judul) }}">
                        @error('judul')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label">Urutan Tampil</label>
                            <input type="number" name="urutan" class="form-control" value="{{ old('urutan', $video->urutan) }}" min="0">
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" rows="2" class="form-control">{{ old('deskripsi', $video->deskripsi) }}</textarea>
                    </div>
                    <button type="submit" class="btn btn-admin-primary w-100"><i class="bi bi-check-lg me-2"></i>Simpan Perubahan</button>
                </form>
            </div>
        </div>
    </div>
...
detectPlatform(@json(old('url', $video->url)));
</script>
@endsection
